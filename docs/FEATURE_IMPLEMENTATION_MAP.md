...
- Board rendering lives in `apps/web/src/Board3D.tsx`
...
| Animations | Implemented, lightweight | `apps/web/src/Board3D.tsx`, `apps/web/src/styles.css` | Board uses a CSS perspective presentation, die cards animate on roll, and dimension previews elevate tiles. |
...
- Match UI labels intentionally append `YOU` / `OPPONENT` in `apps/web/src/App.tsx` so duplicate player names remain readable.
